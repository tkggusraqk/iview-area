...
export default {
  ...
  watch: {
    ...
    value() {
      console.log('value-wtach')
      this.init();
    }
  },
  methods: {
    init() {
      console.log('value-wtach-init:' + JSON.stringify(this.value))
    ...
    updateNextSelector(index, list, nextList, name, nextName, level) {
      console.log('index:' + index + ',list:' + list + ',nextList:' + nextList + ',name:' + name + ',nextName:' + nextName + ',level:' + level)
      if (level <= this.showLevel) {
        let nextSelected = '';
        ...
        if (areaData[this[index]]) {
          ...
          if (this.isInit && this.value.length !== 0) {
            console.log('updateNextSelector:' + JSON.stringify(this.value))
            //   解决自动赋值未设置级别数据
            if (level + 1 < this.cloneValue.length) {
              this[nextName] = nextSelected || this.setNextSelect(index);
            } else {
              this.returnRes(this.value.length);
            }
